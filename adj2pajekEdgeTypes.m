...
function []=adj2pajekEdgeTypes(adj,names,colorsVert,colorsEdge,adjType,typeNames,filename)
...
fprintf(fid,'*Vertices  %6i\n',n);
  ...
  nTypes = size(typeNames,2);
  issym = issymmetric(adj);
  
  for t=1:nTypes
      
        if size(find(adjType(:) == t),1) == 0
            continue;
        end
  
        if issym; fprintf(fid,['*Edges :',int2str(t),' "',typeNames{t},'"\n']); end  % undirected graph version
        if not(issym); fprintf(fid,['*Arcs :',int2str(t),' "',typeNames{t},'"\n']); end  % directed graph version
