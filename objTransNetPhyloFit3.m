function [rec,tree_weight_rec] = objTransNetPhyloFit3(AM_tree,tree,DM,interHostCoeff,aux)
%     sigma = 0.1;
%     epsilon = 0.5;
...
%     interHostCoeff = 0.85;
    rec = 0;
    tree_weight_rec = [];
    for ii = 1:size(interHostCoeff,2)
        ihc = interHostCoeff(ii);
        n = size(DM,2);
        ...
        for i=1:nleafs
            for e = 1:nEdges
                ...
                if tree(edge(1),3) ~= tree(edge(2),3)
                    charVectPaths(i,e) = charVectPaths(i,e)/ihc;
                end
            end
        end
